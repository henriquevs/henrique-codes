...
typedef struct vertice{
   unsigned char num_vertice;
   unsigned short int pos_linha_vertice;
   unsigned short int pos_coluna_vertice;
   unsigned short int numero_pixels;
   unsigned short int min_linha_vertice;
   unsigned short int min_coluna_vertice;
   unsigned short int max_linha_vertice;
   unsigned short int max_coluna_vertice;
   unsigned short int x; // Coluna
   unsigned short int y; // Linha
   struct vertice *prox;
} Vertice;
...
void encontra_vertices(Vertice **p, unsigned char *imagem, int n_linhas, int n_colunas, int i, int j){ // "i" = posicao na linha e "j" = posicao na coluna
   if(get_pos(n_linhas, n_colunas, i, j)==-1 || imagem[get_pos(n_linhas, n_colunas, i+1, j)] != 0) return; // Posicao invalida
...
void encontra_aresta(unsigned char *vertice_A, unsigned char *vertice_B, unsigned char *imagem, int n_linhas, int n_colunas, int i, int j, int numero_de_vertices){ // "i" = posicao na linha e "j" = posicao na coluna
   int aux=get_pos(n_linhas, n_colunas, i, j);
   int indicador;
   ...
   if(aux!=-1 && imagem[aux]==128){
	 ...
	 indicador=imagem[get_pos(n_linhas, n_colunas, i+1, j)];
	 if((*vertice_A)==0) (*vertice_A)=imagem[get_pos(n_linhas, n_colunas, i+1, j)];
	 else if(indicador!=(*vertice_A) && (*vertice_B)==0) {
	 ...
	 if((*vertice_A)==0) (*vertice_A)=imagem[get_pos(n_linhas, n_colunas, i-1, j)];
      ...
      if(imagem[get_pos(n_linhas, n_colunas, i, j+1)]<=numero_de_vertices){  // Verifica se ha vertice abaixo
	 indicador=imagem[get_pos(n_linhas, n_colunas, i, j+1)];
	 if((*vertice_A)==0) (*vertice_A)=imagem[get_pos(n_linhas, n_colunas, i, j+1)];
	 else if(indicador!=(*vertice_A) && (*vertice_B)==0) {
	 ...
	 if((*vertice_A)==0) (*vertice_A)=imagem[get_pos(n_linhas, n_colunas, i, j-1)];
      ...
      if(imagem[get_pos(n_linhas, n_colunas, i+1, j+1)]<=numero_de_vertices){  // Verifica se ha vertice abaixo
	 indicador=imagem[get_pos(n_linhas, n_colunas, i+1, j+1)];
	 if((*vertice_A)==0) (*vertice_A)=imagem[get_pos(n_linhas, n_colunas, i+1, j+1)];
	 else if(indicador!=(*vertice_A) && (*vertice_B)==0) {
	 ...
	 if((*vertice_A)==0) (*vertice_A)=imagem[get_pos(n_linhas, n_colunas, i-1, j-1)];
      ...
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i+1, j, numero_de_vertices); // abaixo
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i-1, j, numero_de_vertices); // acima
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i, j+1, numero_de_vertices); // direita
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i, j-1, numero_de_vertices); // esquerda
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i+1, j+1, numero_de_vertices); // abaixo e direita
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i+1, j-1, numero_de_vertices); // abaixo e esquerda
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i-1, j+1, numero_de_vertices); // acima e direita
      encontra_aresta(vertice_A, vertice_B, imagem, n_linhas, n_colunas, i-1, j-1, numero_de_vertices); // acima e esquerda
...
Vertice *cria_no_vertice(Vertice *p, int n_linhas, int n_colunas){
   ...
   p->numero_pixels=0;
   p->min_linha_vertice=0;
   p->min_coluna_vertice=0;
...
int main(){
   ...
   for(i=0; i<n_linhas; i++)
      for(j=0; j<n_colunas; j++){
	    ...
	    p->min_linha_vertice=i;
	    p->min_coluna_vertice=j;
	    p->max_linha_vertice=i;
	    p->max_coluna_vertice=j;
   ...
   /*for(i=0; i<n_linhas; i++)
	 ...
	 encontra_aresta(&vertice_A, &vertice_B, imagem, n_linhas, n_colunas, i, j, numero_de_vertices);
