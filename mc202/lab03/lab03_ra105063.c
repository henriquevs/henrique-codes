...
void imprime(Caixa *inicio){
   ...
   int flag1=0, flag2=0;
      ...
      if(p->inicio_especial!=NULL && p->fim_especial!=NULL){ /* Se entrarmos neste "if" eh porque a lista de clientes especiais nao eh vazia */
         q=p->inicio_especial;
         r=q;
      ...
      if(p->inicio_normal!=NULL && p->fim_normal!=NULL){ /* Se entrarmos neste "if" eh porque a lista de clientes normais nao eh vazia */
         ...
         if(flag1 == 1){ /* Se entrarmos neste "if" eh porque houve impressao de clientes especiais */
            q=p->inicio_normal;
            r=q;
            printf("->");
            ...
            if(flag1 == 0){ /* Se entrarmos neste "if eh porque NAO houve impressao de clientes especiais */
               q=p->inicio_normal;
               r=q;
               //printf("Visao geral dos clientes:\n");
               printf("Servico %d:\n", p->numero_do_caixa);
...
/* Faz todos as filas avancarem, caso tenham cliente(s), de uma unidade */
Cliente *remover(Caixa *balcao){ /* A liberacao da memoria ocorre na funcao "main" */
   ...
   if(balcao->inicio_especial != NULL){ /* Verifica que a fila de clientes especiais nao esta vazia */
      ...
      if(balcao->inicio_especial == balcao->fim_especial){ /* Remocao quando so existe um cliente na fila */
         ...
         balcao->inicio_especial = balcao->inicio_especial->prox;
         //free(aux);
      }
      return aux;
   }
      ...
      if(balcao->inicio_normal == balcao->fim_normal){ /* Remocao quando so existe um cliente na fila */
         ...
         balcao->inicio_normal = balcao->inicio_normal->prox; /* Atualiza  o inicio da fila de clientes */
         //free(aux);
      }
      return aux;
   }
...
int main(){
   ...
   */
   ...
   } while(opcao != 5); /* encerra expediente */
